...
| A3 | 每次 web 启动恰好多一行账 | 重启后 `wc -l <DSH_HOME>/plugin-boot.jsonl` 递增 1 | 待线上验收 |
| A4 | 判据能抓到「构建晚于启动」 | 重启后构建一个插件 → `plugin_boot_status.stale` 含该插件 | 待线上验收 |
| A5 | 工具输出通过 schema | 调用 `plugin_boot_status` 不报 schema 校验错 | 待线上验收 |
...
- 是否把账本扩展到 watch/headless profile（各自挂载一份，或统一写到同一账本并按 profile 分行）？
